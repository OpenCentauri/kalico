...
#
# Copyright (C) 2026 OpenCentauri Contributors
# Modelled on hx71x.py by Gareth Farrington <gareth@waves.ky> and
# rewritten for Kalico's tap interface by James Turton <james.turton@gmx.com>.
...
#   sample_rate: 1280          # 10 | 40 | 640 | 1280  (default 1280)
#   gain: 128                  # 1 | 2 | 64 | 128       (default 128)
#   ref_output_enable: False   # leave CC2 REFOUT disabled (default)
#
...
DEFAULT_SAMPLE_RATE = 1280
DEFAULT_GAIN = 128
DEFAULT_REF_OUTPUT_ENABLE = False
CS1237_CONFIG_READ = 0x56
CS1237_CONFIG_WRITE = 0x65
CS1237_CONFIG_RETRIES = 10
...
def _make_config_byte(speed_reg, gain_reg, ref_output_enable):
    return ((speed_reg & 0x3) << 4) | ((gain_reg & 0x3) << 2) | (
        0 if ref_output_enable else 0x40
    )
...
class CS1237(LoadCellSensor):
    ...
    def __init__(self, config):
        ...
        self.gain_val = config.getchoice(
            ...
            default=str(DEFAULT_GAIN),
        )
        self.gain_reg = _GAIN_MAP[self.gain_val]
        self.ref_output_enable = config.getboolean(
            "ref_output_enable", default=DEFAULT_REF_OUTPUT_ENABLE
        )
        self.config_byte = _make_config_byte(
            self.speed_reg, self.gain_reg, self.ref_output_enable
        )
        ...
        self.config_cs1237_cmd = None
        self.attach_probe_cmd = None
        mcu.add_config_cmd(
            "config_cs1237 oid=%d drdy_pin=%s sclk_pin=%s"
            % (
                self.oid,
                self.drdy_pin,
                self.sclk_pin,
    ...
    def _build_config(self):
        self.config_cs1237_cmd = self.mcu.lookup_query_command(
            "cs1237_config oid=%c command=%c value=%c",
            "cs1237_config_response oid=%c success=%c value=%c",
            oid=self.oid,
    ...
    def _configure_sensor(self):
        assert self.config_cs1237_cmd is not None
        for _ in range(CS1237_CONFIG_RETRIES):
            write = self.config_cs1237_cmd.send(
                [self.oid, CS1237_CONFIG_WRITE, self.config_byte]
            )
            if not write["success"]:
                continue
            read = self.config_cs1237_cmd.send(
                [self.oid, CS1237_CONFIG_READ, 0]
            )
            if read["success"] and read["value"] == self.config_byte:
                return
        raise self.printer.command_error(
            "%s: CS1237 configuration readback failed" % self.name
        )
